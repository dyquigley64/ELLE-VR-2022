...
public class ELLEAPI : MonoBehaviour
{
    ...
    public static List<LongQuestion> GetCustomQuestionsFromModule(int moduleID)
    {
        string customquestionsJson = MakeRequest("modulequestions", true, new Dictionary<string, string> { { "moduleID", moduleID.ToString() } });

        var temp = SplitJsonArray(customquestionsJson);
        List<LongQuestion> questions = GetCustomQuestionsFromJsonArray(temp);

        List<LongQuestion> customquestions = new List<LongQuestion>();

        for (int i = 0; i < questions.Count; i++)
        {
            if (questions[i].type == "LONGFORM" && questions[i].answers != null)
            {
                customquestions.Add(questions[i]);
                ELLEAPI.GetTextureQuestion(customquestions[customquestions.Count - 1]);
                ELLEAPI.GetAudioClipQuestion(customquestions[customquestions.Count - 1]);
    ...
    private static List<LongQuestion> GetCustomQuestionsFromJsonArray(List<string> questionsJson)
    {
        List<LongQuestion> questions = new List<LongQuestion>();
        for (int i = 0; i < questionsJson.Count; i++)
            questions.Add(JsonConvert.DeserializeObject<LongQuestion>(questionsJson[i]));
        return questions;
    }
    ...
    public static void GetTextureQuestion(LongQuestion question)
    {
        // Coroutines cannot be static. This is my hacky workaround
        var instance = GameObject.Find(GOName).GetComponent<ELLEAPI>();
        instance.StartCoroutine(instance.GetTextureCoroutineQuestion(question));
    }

    public IEnumerator GetTextureCoroutineQuestion(LongQuestion question)
    {
        if (string.IsNullOrEmpty(question.imageLocation)) yield break;

        string url = serverLocation.Remove(serverLocation.LastIndexOf('/'));
        url += question.imageLocation;
    ...
    public static void GetAudioClipQuestion(LongQuestion question)
    {
        // Coroutines cannot be static. This is my hacky workaround
        var instance = GameObject.Find(GOName).GetComponent<ELLEAPI>();
        instance.StartCoroutine(instance.GetAudioClipCoroutineQuestion(question));
    }

    public IEnumerator GetAudioClipCoroutineQuestion(LongQuestion question)
    {
        if (string.IsNullOrEmpty(question.audioLocation)) yield break;

        string url = serverLocation.Remove(serverLocation.LastIndexOf('/'));
        url += question.audioLocation;
        ...
        using (UnityWebRequest www = UnityWebRequestMultimedia.GetAudioClip(url, AudioType.OGGVORBIS))
        {
            ...
            if (www.result == UnityWebRequest.Result.ConnectionError)
                ...
                question.audio = DownloadHandlerAudioClip.GetContent(www);
        }
    }
}